...
**Scope.** 298 files / ~99,593 lines. Covers everything in the working tree except: the
...
| `CHANGELOG.md` | 1894 | prose changelog newest-first: a large unreleased section over v1.4 back to v0.1, feature by feature | docs | active |
...
| `ROADMAP.md` | 552 | lane-claiming roadmap: shipped/building/decided/open items per subsystem, plus the 2.0 editing principles | docs | active |
...
| `docs/subsystems/remote-mobile.md` | 960 | subsystem doc #4 (CV-X/Matrox remotes + phone view): the reverse-engineered CV-X wire protocol whose reference client is gone, the vendor-reflected mouse enum, the only listening socket's trust posture, and the app's one UAC elevation | docs | active |
...
| `src/backupviewer/api.py` | 5248 | the pywebview bridge class: 131 @_endpoint methods returning {ok,data} envelopes across every feature area | shared/infra | active |
...
| `src/backupviewer/discover.py` | 997 | subnet scan job finding fanuc/keyence over ftp and matrox via ethernet/ip, plus adapter list and live name probe | backup capture | active |
...
| `src/backupviewer/web/css/components.css` | 2877 | the app's single component stylesheet: cards, tables, pills, modals, plus per-tab styles for every subsystem | theming | active |
...
| `src/backupviewer/web/index.html` | 157 | the single page: the two chrome rows (topbar with the screens button, per-screen toolbar), #view, jobstrip, statusbar, and the ordered script list | shared/infra | active |
...
| `src/backupviewer/web/js/components/camfeed.js` | 454 | BV.camFeed: the one beat behind every live camera picture - per-img load lifecycle, honesty notes, retry backoff, the rotating per-beat fetch budget and the shared CV-X tile-lease map, fed by any number of surfaces | cameras | active |
...
| `src/backupviewer/web/js/camfloat.js` | 590 | BV.camFloats: the camera wall's floating boxes - pop a tile out, swap which camera a box shows, arrange and lock them; fed by camfeed so a float and its tile share one CV-X session | cameras | active |
| `src/backupviewer/web/js/cvxremote.js` | 308 | cv-x remote overlay: mjpeg screen mirror plus full mouse forwarding, session adopt/rebind for pop-outs | remote/mobile | active |
...
| `src/backupviewer/web/js/keys.js` | 155 | global keydown map (number-row tabs, ctrl+k/e, j/k, esc, backspace) plus the shortcuts help modal | shared/infra | active |
...
| `src/backupviewer/web/js/mtxremote.js` | 257 | matrox remote overlay: sandboxed iframe tabs of the camera's own web ui, window fallback if unframeable | remote/mobile | active |
...
| `src/backupviewer/web/js/router.js` | 532 | hash router and app boot: the screens dropdown + breadcrumb feed, chrome edge hairlines, cubes, status bar, boot sequence | shared/infra | active |
...
| `src/backupviewer/web/js/tabs/home.js` | 3575 | the #home library screen: plant/line/robot tree, per-row actions, batch ftp backup, discover, cam tiles | library | active |
...
| `src/backupviewer/web/js/netstatus.js` | 419 | plant-link pill + drop panel: 4-state link chip, segment device list merged with the library, gentle arp recheck | backup capture | active |
...
| `src/backupviewer/web/js/util.js` | 548 | boots window.BV: esc/el/fmt/toast/copy, host-window fullscreen, modal+dirtyGuard, menu/dropPanel, collapsible | shared/infra | active |
...
| `tests/test_discover.py` | 670 | pytest for backup-folder detection and the offline network scan: fanuc ftp, matrox ethernet/ip, adapters | tests | active |
...
| `tests/ui_camfloat_probe.py` | 916 | hidden-window probe: the floating camera boxes - layer paint order under frost, pop-out costs no dial, snap/lock/resize geometry, parking off the library | tests | active |
...
| `tests/ui_cvxremote_probe.py` | 300 | hidden-window probe of the cv-x remote bar, top-bar phone button and pop-out session adoption | tests | active |
| `tests/ui_cleanup_probe.py` | 348 | hidden-window probe of the manage-backups modal: report/cleanup tabs, verdict folds, pin round-trip, two-click stage with real `_staged` moves, auto-stage sweep | tests | active |
