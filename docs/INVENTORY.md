...
**Scope.** 298 files / ~99,504 lines. Covers everything in the working tree except: the
...
| `CHANGELOG.md` | 1883 | prose changelog newest-first: a large unreleased section over v1.4 back to v0.1, feature by feature | docs | active |
...
| `docs/subsystems/remote-mobile.md` | 951 | subsystem doc #4 (CV-X/Matrox remotes + phone view): the reverse-engineered CV-X wire protocol whose reference client is gone, the vendor-reflected mouse enum, the only listening socket's trust posture, and the app's one UAC elevation | docs | active |
...
| `src/backupviewer/web/js/components/camfeed.js` | 428 | BV.camFeed: the one beat behind every live camera picture - per-img load lifecycle, honesty notes, retry backoff, the rotating per-beat fetch budget and the shared CV-X tile-lease map, fed by any number of surfaces | cameras | active |
...
| `src/backupviewer/web/js/camfloat.js` | 581 | BV.camFloats: the camera wall's floating boxes - pop a tile out, swap which camera a box shows, arrange and lock them; fed by camfeed so a float and its tile share one CV-X session | cameras | active |
...
| `tests/ui_camfloat_probe.py` | 882 | hidden-window probe: the floating camera boxes - layer paint order under frost, pop-out costs no dial, snap/lock/resize geometry, parking off the library | tests | active |
