...
**Scope.** 276 files / ~81,293 lines. Covers everything in the working tree except: the
...
| `CHANGELOG.md` | 1304 | prose changelog newest-first: a large unreleased section over v1.4 back to v0.1, feature by feature | docs | active |
...
| `docs/INVENTORY.md` | 645 | this file: dated file-level map of the repo plus the findings from the phase-1 documentation pass, with resolved items marked inline | docs | active |
...
| `src/backupviewer/web/css/components.css` | 2390 | the app's single component stylesheet: cards, tables, pills, modals, plus per-tab styles for every subsystem | theming | active |
...
| `src/backupviewer/web/js/util.js` | 442 | boots window.BV: esc/el/fmt/toast/copy, host-window fullscreen, modal+dirtyGuard, menu/dropPanel, collapsible | shared/infra | active |
...
| `tests/test_cvx_stream.py` | 125 | pytest for the mjpeg bridge's flush behavior: a real session against the loopback sim, a raw http client reading the stream — one frame in, the frame plus its idle flush out, and never a third | tests | active |
