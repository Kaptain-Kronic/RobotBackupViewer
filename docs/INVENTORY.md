...
**Scope.** 259 files / ~74,282 lines. Covers everything in the working tree except: the
...
| `CHANGELOG.md` | 1164 | prose changelog newest-first: a large unreleased section over v1.4 back to v0.1, feature by feature | docs | active |
...
| `src/backupviewer/api.py` | 4106 | the pywebview bridge class: 117 @_endpoint methods returning {ok,data} envelopes across every feature area | shared/infra | active |
...
| `src/backupviewer/parsers/__init__.py` | 34 | parsers package init holding TAB_REQUIREMENTS, the map of which backup files make each ui tab available | backup parsing | active |
...
| `src/backupviewer/session.py` | 462 | BackupSession: recursive case-insensitive file index, lazy parse cache, ls/karel classify, backup-type sniff | backup parsing | active |
...
| `src/backupviewer/web/index.html` | 147 | the single page: the two chrome rows (topbar with the screens button, per-screen toolbar), #view, jobstrip, statusbar, and the ordered script list | shared/infra | active |
| `src/backupviewer/web/js/api.js` | 143 | promise wrapper over the pywebview bridge: {ok,data} envelope, solo/cvx-window sid injection, slow-call dedupe | shared/infra | active |
