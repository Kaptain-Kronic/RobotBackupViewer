...
def test_scrape_relative_href_with_pgx_stripped():
    html = '<a href="DesignAssistant/GMDAX_9_1_MULTI_FANUC_G_V2_0/default.htm?pgx=0.858566">op</a>'
    pages = _find_da_pages("198.51.100.50", html)
    assert pages == [{
        "label": "design assistant",
        "url": "http://198.51.100.50/DesignAssistant/GMDAX_9_1_MULTI_FANUC_G_V2_0/default.htm",
    }]
...
def test_scrape_da9_project_row_unquoted_attr():
    ...
    html = ("<tr class='project-inf-cntr bottom-row-round' "
            "prj-name=GMDAX_9_1_MULTI_FANUC_G_V2_0>\n<td>GMDAX_9_1_MULTI_FANUC_G_V2_0</td>")
    pages = _find_da_pages("198.51.100.50", html)
    assert pages == [{
        "label": "design assistant",
        "url": "http://198.51.100.50/DesignAssistant/GMDAX_9_1_MULTI_FANUC_G_V2_0/default.htm",
...
def test_window_creates_a_webview_window(monkeypatch):
    ...
    r = Api().mtx_remote_window({"ip": "10.1.2.3", "label": "DD187 CAM"})
    assert r["ok"] is True
    assert made == [("MTX remote · DD187 CAM", "http://10.1.2.3/")]
